...
def run(all_ids: list):
    ...
    p = processing(output_path="data/feature_space/test.csv", bag_data_folder="data/bag_data")
    p.run()

    # Calculate the similarity between the data
    print(p.output_path)
    s = similarity(feature_space_path=p.output_path, columns= ["roughness_index_3d", "actual_volume"])
